...
# Helpers
# ----------------------------
TAIL_REGEX = re.compile(r"^C-[A-Z0-9]{4}$")   # Simple Canadian tail check like C-FASV
FAKE_TAIL_PATTERNS = [
...
def is_real_tail(tail: str) -> bool:
    ...
    for pat in FAKE_TAIL_PATTERNS:
        if pat.search(tail):
            return False
    return bool(TAIL_REGEX.match(tail.strip()))
...
def parse_utc_ddmmyyyy_hhmmz(series: pd.Series) -> pd.Series:
    ...
    """
    s = series.astype(str).str.strip().str.replace("z", "", regex=False)
...
def type_badge(flight_type: str) -> str:
    # Basic color-coding via emoji badges (reliable in Streamlit tables)
...
if uploaded is not None:
    ...
    # Expecting these column names (from your sample):
    # Booking, Off-Block (Est), On-Block (Est), From (ICAO), To (ICAO),
    # Flight time (Est), PIC, SIC, Account, Aircraft, Aircraft Type, Workflow
    missing = [c for c in [
        "Booking", "Off-Block (Est)", "On-Block (Est)", "From (ICAO)", "To (ICAO)",
        "Flight time (Est)", "PIC", "SIC", "Account", "Aircraft", "Aircraft Type", "Workflow"
    ...
    df["Departs In"] = (df["ETD_UTC"] - now_utc)
    df["Arrives In"] = (df["ETA_UTC"] - now_utc)
    df["Departs In"] = df["Departs In"].apply(fmt_td)
    df["Arrives In"] = df["Arrives In"].apply(fmt_td)
...
st.dataframe(df[display_cols], use_container_width=True)
    ...
    with st.expander("Simulate a FlightBridge email alert → update status / detect delays"):
        ...
        booking_choices = df["Booking"].unique().tolist()
        if booking_choices:
            ...
            if st.button("Apply Update"):
                ...
                row = df[df["Booking"] == sel_booking].iloc[0]
                planned = row["ETD_UTC"] if update_type == "Departure" else row["ETA_UTC"]
                delta_min = None
