...
def _get_fl3xx_schedule(
    ...
    if not config.auth_header and not config.api_token:
        raise RuntimeError(
            ...
            "or when your API token must be sent under a different header."
        )
...
if selected_source == "csv_upload":
    ...
    if not config.auth_header and not config.api_token:
        st.error(
            ...
            "different header name. When combined with 'api_token' the raw token will "
            "be sent under the provided header."
        )
        st.stop()
